...
Persona* crearPersona(char nombre[], char rut[], int edad) {
  Persona* p = calloc(1, sizeof(Persona));
  if (!p) { return(NULL); }
...
void sumaV(Vector * a, Vector * b, Vector * c) {
  ...
  for(i = 0; i < c->capacidad; ++i) {
    // Asumo que para esto creamos ambas funciones
    asignarValor(c, i, obtenerValor(a, i) + obtenerValor(b, i));
  }
}
